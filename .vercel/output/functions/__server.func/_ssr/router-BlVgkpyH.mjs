...
//#region node_modules/.nitro/vite/services/ssr/assets/router-BlVgkpyH.js
...
var styles_default = "/assets/styles-DImTuCf-.css";
...
var Route$6 = createFileRoute("/api/chat")({ server: { handlers: { POST: async ({ request }) => {
	...
	try {
		...
		if (apiKey && apiKey !== "your_openai_api_key_here") try {
			...
			try {
				...
				if (parsed.error?.message) apiErrMessage = `OpenAI API (${parsed.error.type || response.status}): ${parsed.error.message}`;
			} catch (_) {}
			return new Response(JSON.stringify({ reply: apiErrMessage }), { headers: { "Content-Type": "application/json" } });
		} catch (openAiErr) {
			console.error("[OpenAI Network Error]:", openAiErr);
			const fallbackAnswer = getFallbackReply(messages);
			return new Response(JSON.stringify({ reply: `${fallbackAnswer}\n\n*(Note: Live OpenAI API connection unavailable — getaddrinfo ENOTFOUND api.openai.com)*` }), { headers: { "Content-Type": "application/json" } });
		}
...
function getSessionSecret() {
	...
	if (!secret || secret === "your_random_32_character_secret_key_here") return "dev-local-session-secret-fallback-chanakya-2026";
	return secret;
}
/**
...
function createSignedSessionToken(username) {
	...
	return `${payloadB64}.${crypto$1.createHmac("sha256", secret).update(payloadB64).digest("hex")}`;
}
/**
...
function verifySignedSessionToken(token) {
	...
	const secret = getSessionSecret();
	const expectedSignature = crypto$1.createHmac("sha256", secret).update(payloadB64).digest("hex");
	const sigBuffer = Buffer.from(signature, "hex");
	const expectedBuffer = Buffer.from(expectedSignature, "hex");
	if (sigBuffer.length !== expectedBuffer.length || !crypto$1.timingSafeEqual(sigBuffer, expectedBuffer)) return false;
	try {
